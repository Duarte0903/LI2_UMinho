...
{
    int r = 0;
    if (convertParaInt (s, token) || lerLinha (s, token) || intParaChar (s, token) || troca2Topo (s, token) || copiaNesimo (s, token) || popG2 (s, token) || duplicar (s, token) || rodar3 (s, token)) r = 1;
    return r;
}
...
{
    int r = 0;
    if (lerInput (s, token) || pushEspaco (s, token) || criaString (s, token) || abreParReto (s, token, flagArrays) || fechaParReto (s, token, flagArrays) || range (s, token)) r = 1;
    return r;
}
...
{
    int r = 0;
    if (handleG3 (s, token) || handleG2 (s, token) || handleG1 (s, token) || handleG4 (s, token, flagArrays)) r = 1;
    return r;
}
...
void parser (STACK *s, char *token)
{
    ...
    if (strlen (sobra) == 0)                                /*!< Testa se o TOKEN é LONG */
    {
    ...
    else
    {
        ...
        if (strlen (sobra) == 0)                           /*!< Testa se o TOKEN é DOUBLE */
        {
